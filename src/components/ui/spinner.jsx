...
export const Spinner = ({
  ...
  return (
    <div aria-label="Loading..." role="status">
      <Loader className={cn("animate-spin", sizesClasses[size], strokeClasses[color])} />
    </div>
  );
}
  ...
  <div className="w-fit">
    <div className="relative flex size-full items-center justify-start">
      <motion.span
        ...
        className="absolute left-2 top-0 size-3.5 rounded-full bg-current"></motion.span>
      <motion.span
        initial={{ x: 0 }}
        animate={{ x: 24 }}
        transition={{ duration: 0.7, repeat: Infinity }}
      ...
      <motion.span
        ...
        animate={{ x: 24 }}
        transition={{ duration: 0.7, repeat: Infinity }}
        className="absolute left-8 top-0 size-3.5 rounded-full bg-current"></motion.span>
      <motion.span
        initial={{ scale: 1 }}
        animate={{ scale: 0 }}
        transition={{ duration: 0.7, repeat: Infinity }}
        className="absolute left-14 top-0 size-3.5 rounded-full bg-current"></motion.span>
    </div>
  </div>
)
...
export const Dots_v3 = () => {
  return (
    <div className="flex items-center justify-center space-x-2">
      <div
        className="size-3.5 animate-bounce rounded-full bg-current [animation-delay:-0.3s]"></div>
      <div
        className="size-3.5 animate-bounce rounded-full bg-current [animation-delay:-0.13s]"></div>
